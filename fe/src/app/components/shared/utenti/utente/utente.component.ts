...
export class UtenteComponent implements OnInit {
  ...
  constructor(private activatedRoute: ActivatedRoute,
    ...
    private usrSvc:UtenteApiService,
  ...
  createForm() {
    this.activatedRoute.queryParams.subscribe(params => {
      let idUtente = params['id'];
      if( idUtente && idUtente !== null && idUtente !== "" ){
        this.usrSvc.get(CONST.USERS_PROTECTED_URL.endsWith("/")?CONST.USERS_PROTECTED_URL+idUtente:CONST.USERS_PROTECTED_URL+"/"+idUtente).subscribe({
          next: (response) =>{
            this.utente = response.payload[0];
            this.createTheForm(this.utente);
          },
          error: (error) => {
            this.messageService.add({severity:'error',summary:"Recupero dati utente",detail:"Errore recupero dati utente"})
          }
        });
      }else{
      this.createTheForm(this.utente);
  ...
  private createTheForm(utente:Utente){
    this.nome = new FormControl(utente.nome, { validators: Validators.required });
      this.cognome = new FormControl(utente.cognome, { validators: Validators.required });
      this.codiceFiscale = new FormControl(utente.codiceFiscale, { validators: [Validators.required, Validators.pattern("^[A-Za-z]{6}[0-9]{2}[A-Za-z]{1}[0-9]{2}[A-Za-z]{1}[0-9]{3}[A-Za-z]{1}$")] });
      this.dataNascita = new FormControl(utente.dataNascita, { validators: Validators.required });
      this.sessoFc = new FormControl(this.getSessoUtente, { validators: Validators.required });
      this.indirizzo = new FormControl(utente.indirizzo, { validators: Validators.required });
      this.username = new FormControl(utente.username, { validators: [Validators.required, Validators.minLength(3)], asyncValidators: [AsyncValidators.validateUsername(this.initDb)] });
